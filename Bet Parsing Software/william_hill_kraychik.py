...
def extractRowsFromBlock(block):
	...
	if (len(f) == 0):		# ни одной строчки не найдено
		ThrowError(0, "extractRowsFromBlock(block): Регулярное выражение не смогло извлечь информацию о рядах данных. Программа не прерывается.")
	else:
		for i in range(len(f)):
			row = block.find_by_id(f[i])
			if row == None:			# ошибка! очень странная, никогда не должна возникать
				ThrowError(1, "extractRowsFromBlock(block): не удалось найти ряд по id = " + str(f[i]))
			else:
				rows.append(row)
	return rows
...
def extractInfoFromRow(row):
	...
	if cmds[0] == "" or cmds[1] == "":
		ThrowError(1, "extractInfoFromRow(row): не пройдена проверка корректности имен команд. Программа прервана.")
...
def extractDateTimeFromRow(row_html):
	...
	f = re.findall(c, row_html)
	if len(f) == 0:
		ThrowError(0, "extractDateTimeFromRow(row_html): время не найдено. Программа не прервана.")
	else:
		h = int(f[0][0])
		mint = int(f[0][1])
	return datetime.datetime(y, mon, d, h, mint)
...
def mapMonth(s):
	if s == "Jan":
		...
		ThrowError(1, "mapMonth(s): строка s == " + s + " не соответствует ни одному образцу. Программа прервана.")
		return -1
...
def getInfo(allBlocks):
	...
	for i in range(len(allBlocks)):
		...
		if len(rows) == 0:		# подозрительно на ошибку. в принципе такой ситуации быть не должно
			ThrowError(0, "getInfo(allBlocks): warning: массив rows пуст")
		else:
