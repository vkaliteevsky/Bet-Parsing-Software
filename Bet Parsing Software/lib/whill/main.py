...
def initParser(state_file, amount_of_pages):
	...
	writeState(state_file, [str(now_new), str(now_old), 0])
	return 0
...
def readState(state_file):
	...
	if not os.path.exists(state_file):
		dt = datetime.datetime(1970, 1, 1, 0, 0, 0).strftime("%Y-%m-%d %H:%M:%S\n")
		#lines.append(str())
		#lines.append(str(datetime(1970, 1, 1, 0, 0, 0)))
		#lines.append(str(0))
		writeState(state_file, [str(dt), str(dt), str(0)])
	f = open(state_file, "r")
	line = f.readline()
	while (line):
		lines.append(line)
