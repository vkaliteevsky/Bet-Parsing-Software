...
def initParser(state_file, amount_of_pages):
	...
	writeState(state_file, [str(now_old)+"\n", str(now_new)+"\n", 0])
	return 0
...
def parseAll(min_minutes, max_minutes, amount, folder_name, state_file, input_folder):
	...
	while link:
		...
		if min_minutes <= delta_mins and delta_mins <= max_minutes:		# фильтрация по времени
			subprocess.Popen(["node", "parse_page.js", link, folder_name + str(counter), cmd1, cmd2])
			counter += 1
		dt_str = dates_file.readline()
		link = links_file.readline()
		cmd1 = cmds_file.readline()
...
def main():
	...
	if cmd == "init":
		...
		print readState("state.stf")
		return readState("state.stf")
	elif cmd == "parseMatch":
		dt_str = sys.argv[2]
		cmd1 = sys.argv[3]
		cmd2 = sys.argv[4]
		parseMatch(dt_str, cmd1, cmd2, "all_result", "state.stf", "running")
	else:
